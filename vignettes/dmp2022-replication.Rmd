...
the $(\bar r_X, \bar r_Y)$ plane. That is what
...
$\bar c = 1$, the breakdown point from Table 1: as $\bar r_Y$ grows the
restriction on the outcome side stops binding and the frontier is the
$\bar r_X$ breakdown point under no restriction at all. The horizontal
arm is the level of $\bar r_Y$ below which the sign survives *any*
$\bar r_X$. Smaller $\bar c$ pushes both out, because restricting how
endogenous the controls may be makes the conclusion harder to overturn.

Reading the frontier in the other direction shows why the package offers
both. Fix $\bar r_Y$ and ask for the $\bar r_X$ at which the sign fails:
just above the horizontal arm there is an answer, and on it there is
none, because no $\bar r_X$ overturns the sign there.
