...
```{r bp-comparison}
compare_published <- function(quantity, published, computed) {
    out <- data.frame(
        quantity  = quantity,
        published = published,
    ...
    # The paper rounds to the printed precision, so agreement is judged
    # against that rather than against machine precision.
    tol <- 10^(-nchar(sub("^[^.]*\\.", "", format(published)))) / 2
    out$agrees <- abs(out$difference) <= tol
    out
}
