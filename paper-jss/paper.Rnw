...
$\Sexpr{nrow(rho)}$ calibration values. Only \code{ave_gyi} and
\code{d_coa} are more important for treatment selection than the
unobservable would have to be. If one regards the $\rho_k$ as plausible
magnitudes for $r_X$, the finding is fairly robust.
...
$\Sexpr{fmt(boot$ci[1], 3)}$, still exceeds all but two of the $\rho_k$
...
as $95.9$\% by DMP. Every entry agrees to within $0.06$, and the pattern
...
lies \emph{below} both. An unobservable no more important for class-size
selection than the free-lunch share is already enough to overturn the sign.
...
Section~\ref{sec:api}---where the breakdown point exceeded eight of ten
...
The package, with its three vignettes---a tour, a paper-replication
walk-through, and a stylized illustration of the Masten--Poirier
