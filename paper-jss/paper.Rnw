...
## Layout mandated by the JSS style guide.
...
## Helper used to inject computed numbers into the prose via \Sexpr{}, so that
## no quoted value in the text can drift away from the code that produces it.
...
<<api-bounds-vals, echo = FALSE>>=
bp_c01 <- bnds$breakdown
bp_c00 <- regsen_breakdown(form, bfg2020, compare = w1, cbar = 0)$results$breakdown[1]
bp_c10 <- regsen_breakdown(form, bfg2020, compare = w1, cbar = 1)$results$breakdown[1]
@

The header reports the estimation sample ($n = \Sexpr{bnds$n}$ counties),
...
Note $\hat\bmed = \Sexpr{fmt(bnds$dgp$beta_med, 4)}$, which is the value
DMP Table 1 column (5) reports as $2.055$. The breakdown point at
$\cbar = 0.1$ is $\Sexpr{fmt(bp_c01, 4)}$: the smallest $\rxbar$ at which
...
$\Sexpr{fmt(bp_c10, 4)}$ and at $\cbar = 0$ it rises to
$\Sexpr{fmt(bp_c00, 4)}$.
...
unobservables up to $\Sexpr{pct(bp_c00)}$\% of selection on observables;
under arbitrarily endogenous controls only up to
$\Sexpr{pct(bp_c10)}$\%. A researcher who reported only the former would
overstate robustness by a factor of about
$\Sexpr{fmt(bp_c00 / bp_c10, 1)}$.
...
$\Sexpr{pct(bp_c10)}$\%, which exceeds
$\Sexpr{sum(rho$rho < 100 * bp_c10)}$ of the
$\Sexpr{nrow(rho)}$ calibration values. Only \code{ave_gyi} is more
...
$\Sexpr{fmt(boot$ci[1], 3)}$, still exceeds
$\Sexpr{sum(rho$rho < 100 * boot$ci[1])}$ of the
$\Sexpr{nrow(rho)}$ $\rho_k$ values above, so the qualitative
conclusion does not rest on the point estimate alone. (The count is
computed from the interval, not asserted: the nearest $\rho_k$ sits
close to the endpoint, and a different seed could move it across.) Omitting \code{cluster} gives the i.i.d.\ bootstrap, which
...
<<api-frontier-plot, echo = FALSE, fig.cap = "Estimated breakdown frontiers for the hypothesis beta-long > 0. Each curve fixes a bound cbar on control endogeneity and traces the largest rxbar consistent with the conclusion, as a function of rybar. Pairs below and left of a curve are robust. Tightening cbar shifts the frontier outward, so assumptions about control endogeneity and about selection strength are substitutes.">>=
fr <- frontier[is.finite(frontier$rybar) & !is.na(frontier$rxbar_bp), ]
ggplot(fr, aes(x = rxbar_bp, y = rybar,
...
$\rxbar < \Sexpr{fmt(max(fr$rxbar_bp[fr$cbar == 1]), 2)}$, the finiteness
threshold at that $\cbar$. The \code{try()} guard in the loop is
defensive: on this grid no call fails, because
\code{regsen\_breakdown()} solves along the boundary of the
characterized region, but \code{regsen\_bounds()} does raise an error
for points beyond it -- the region Section~\ref{sec:regimes} described
as uncharacterized -- and the guard costs nothing on grids that stray
there. Users who do hit that error should not mistake it for a
convergence failure.
...
<<replication-compute, echo = FALSE>>=
inp <- regsensitivity:::build_dgp_inputs(form, bfg2020, compare = w1)
dgp <- regsensitivity:::get_dgp(inp)
r2rot <- min(dgp$r_med * 1.3, 1)
o_del <- regsen_breakdown(form, bfg2020, compare = w1, analysis = "oster",
                          r2long = r2rot,
                          beta = bnd_eq(0))$results$breakdown[1]
rbar_bp <- regsen_breakdown(form, bfg2020, compare = w1, cbar = 1,
                            rybar_expr = function(rx) rx)$results$breakdown[1]
dgp_bp <- regsen_bounds(form, bfg2020, compare = w1, cbar = 1)$breakdown
@
...
$\hat{\overline{r}}^{\mathrm{bp}}$ of $\Sexpr{pct(rbar_bp)}$\% is printed
...
<<replication-table, results = "asis", echo = FALSE>>=
rep_tab <- data.frame(
    Quantity = c("$\\hat\\beta_{\\mathrm{med}}$",
                 "Number of counties",
                 "Mean of dependent variable",
                 "$\\hat\\delta^{\\mathrm{bp}}_{\\mathrm{resid}}$ (corrected)",
                 "$\\hat{\\overline{r}}^{\\mathrm{bp}}_X$ (\\%)",
                 "$\\hat{\\overline{r}}^{\\mathrm{bp}}$ (\\%)"),
    Published = c("2.055", "2,036", "60.04", "-23.3", "80.4", "95.9"),
    Package = c(fmt(dgp$beta_med, 3), format(dgp$n, big.mark = ","),
                fmt(mean(bfg2020$avgrep2000to2016, na.rm = TRUE), 2),
                fmt(o_del, 1), pct(dgp_bp), pct(rbar_bp)),
    Source = c("Table 1, Panel A", "Table 1, Panel A", "Table 1, Panel A",
               "Table 1, Panel B", "Table 1, Panel C", "Table 1, Panel C"),
    check.names = FALSE
)
print(xtable::xtable(rep_tab,
    ...
    booktabs = TRUE, table.placement = "t!", comment = FALSE)
@

Two of these deserve comment. The corrected Oster delta of
$\Sexpr{fmt(o_del, 1)}$ reproduces DMP's correction of the published
...
$\hat{\overline{r}}^{\mathrm{bp}}_X = \Sexpr{pct(dgp_bp)}\%
\le \hat{\overline{r}}^{\mathrm{bp}} = \Sexpr{pct(rbar_bp)}\%$
...
<<case-school-extra, echo = FALSE>>=
sch_bp0 <- regsen_breakdown(f, CASchools, compare = c("english", "lunch"),
                            cbar = 0)$results$breakdown[1]
sch_rho <- calibrate_rho(f, CASchools, compare = c("english", "lunch"))
sch_boot <- regsen_boot(f, CASchools, compare = c("english", "lunch"),
                        cbar = 1, R = 199, seed = 7, show_progress = FALSE)
@

The medium regression gives
$\hat\bmed = \Sexpr{fmt(res$dgp$beta_med, 3)}$: a one-unit increase in the
student--teacher ratio is associated with a
$\Sexpr{fmt(abs(res$dgp$beta_med), 2)}$-point lower average score. The
breakdown point at $\cbar = 1$ is
$\Sexpr{fmt(res$breakdown, 3)}$, so the negative-effect conclusion survives
unless selection on unobservables is at least
$\Sexpr{pct(res$breakdown)}$\% as strong as selection on the two
...
values are $\Sexpr{fmt(sch_rho$rho[1], 1)}$\% and
$\Sexpr{fmt(sch_rho$rho[2], 1)}$\%, so the breakdown point at $\cbar = 1$
...
overturn the sign.
The bootstrap interval, $[\Sexpr{fmt(sch_boot$ci[1], 3)},
\Sexpr{fmt(sch_boot$ci[2], 3)}]$ over $R = 199$ i.i.d.\ replicates, does
...
<<sim-table, results = "asis", echo = FALSE>>=
st <- sim_tab
names(st) <- c("$n$", "Coverage (\\%)", "Bias", "SD", "Mean CI width")
print(xtable::xtable(st,
    caption = paste0("Finite-sample performance of the percentile bootstrap ",
      "interval for the breakdown point, at nominal 95\\% level. ",
      "$M = ", M, "$ Monte Carlo replications, $R = ", Rb, "$",
      " bootstrap draws each, $\\bar c = 1$. The pseudo-true breakdown ",
      "point is ", fmt(truth, 4), ", computed on a single sample of ",
      "$4 \\times 10^5$ observations. Bias and SD refer to the point ",
      "estimator of the breakdown point."),
    label = "tab:coverage", digits = c(0, 0, 1, 4, 4, 4)),
    include.rownames = FALSE, sanitize.text.function = identity,
    booktabs = TRUE, table.placement = "t!", comment = FALSE)
@
...
$\Sexpr{fmt(sim_tab$coverage[1], 1)}$\% against a nominal $95$\%. The
...
<<compare-vals, echo = FALSE>>=
rv <- sm$sensitivity_stats$rv_q
r2yd <- sm$sensitivity_stats$r2yd.x
@

The robustness value is $RV_{q=1} = \Sexpr{fmt(rv, 4)}$: an unobservable
explaining more than $\Sexpr{pct(rv)}$\% of the residual variation in
\emph{both} treatment and outcome would suffice to bring the estimate to
zero. The partial $R^2$ of treatment with outcome is
$\Sexpr{fmt(r2yd, 4)}$. \pkg{regsensitivity} reports, for the same
specification, a breakdown point of $\Sexpr{pct(bp_c10)}$\% at
$\cbar = 1$.
...
\proglang{R}~\Sexpr{paste(R.version$major, R.version$minor, sep = ".")}
on \Sexpr{R.version$platform}, with the packages
\pkg{regsensitivity}~\Sexpr{packageVersion("regsensitivity")},
\pkg{ggplot2}~\Sexpr{packageVersion("ggplot2")},
\pkg{nloptr}~\Sexpr{packageVersion("nloptr")},
\pkg{AER}~\Sexpr{packageVersion("AER")},
\pkg{sensemakr}~\Sexpr{packageVersion("sensemakr")} and
